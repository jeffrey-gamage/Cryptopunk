...
public class PathPreview : MonoBehaviour
{
    ...
    internal void DisplayPreview(List<DungeonTile> previewPath)
    {
        ...
        if (previewPath.Count > 1&&!(DungeonManager.instance.mode == DungeonManager.Mode.Wait))
        {
            ...
            {
                pathTrace.Add(Instantiate(pathSegment, previewPath[i].GetOccupyingCoordinates(false,true)+Vector3.up*pathPreviewOffset, previewPath[i].getPreviewRotation()).GetComponent<SpriteRenderer>());
            }
        }
        else
        {
